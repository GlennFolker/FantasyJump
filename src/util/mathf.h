...
namespace Fantasy {
    class Mathf {
        ...
        static inline void randVecs(unsigned int seed, int amount, float maxLength, float progress, const std::function<void(float, float)> &func) {
            randVecs(seed, amount, maxLength, progress, 0.0f, glm::two_pi<float>(), func);
        }

        static inline void randVecs(unsigned int seed, int amount, float maxLength, float progress, float coneFrom, float coneTo, const std::function<void(float, float)> &func) {
            srand(seed);
            for(int i = 0; i < amount; i++) {
                float angle = random(coneFrom, coneTo);
                float len = (random() * maxLength) * progress;
